...
export function useLightBallGame() {
  ...
  // ========== 创建粒子 ==========
  function createParticles(position: Position, color: string): void {
  ...
  function updateBallPosition(): void {
    ...
    const w = GAME_CONFIG.SCREEN_WIDTH;
    const h = GAME_CONFIG.SCREEN_HEIGHT;
    ...
    if (collided && collisionPosition) {
      createParticles(collisionPosition, BALL_CONFIG.COLOR);
    }
  }
